...
# Güçlendirilmiş deneme
MAX_TRAIN_SAMPLES = 2000
MAX_VALID_SAMPLES = 600
...
def save_training_curves(history):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Value")
    plt.title("Fine-Tuning Training and Validation Results")
    plt.legend()
    plt.grid(True)

    save_path = FIGURE_DIR / "training_curves_finetune.png"
...
def save_confusion_matrix(labels, predictions):
    ...
    save_path = FIGURE_DIR / "confusion_matrix_finetune.png"
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def save_classification_report(labels, predictions):
    ...
    save_path = REPORT_DIR / "classification_report_finetune.txt"
...
def main():
    ...
    # Son blok olan layer4'ü eğitime açıyoruz.
    ...
    # Abnormal sınıfını biraz daha önemli hale getiriyoruz.
    ...
    for epoch in range(EPOCHS):
        ...
        valid_loss, valid_acc, labels, predictions = validate(
            model,
            valid_loader,
            criterion,
            device
        ...
        if valid_acc > best_valid_acc:
            ...
            save_path = MODEL_DIR / "finetuned_resnet18.pth"
            torch.save(model.state_dict(), save_path)
            print("En iyi fine-tuned model kaydedildi:", save_path)
    ...
    print("\nFine-tuning eğitimi tamamlandı.")
    print("En iyi validation accuracy:", round(best_valid_acc, 4))
